...
- **Filtro por rango de fechas** en las columnas de fecha y fecha y hora (calendario con día inicial y final, ambos incluidos; se aplica con el botón **Aplicar** y muestra el rango elegido).
...
**Columnas de fecha y de fecha y hora:** el panel del embudo muestra un **calendario de rango** y el rango elegido escrito en texto (*Rango seleccionado: 22/09/2026 → 25/09/2026*). Se elige el **día inicial** y después el **día final**, y al pulsar **Aplicar** se muestran los registros de ambos días y los intermedios (los dos extremos están incluidos, comparando la fecha real del registro y no el texto que se ve en la celda). Mientras solo hay un extremo elegido, la fila del rango muestra solo el primer día y el panel recuerda que falta el último.

Nada se filtra hasta pulsar **Aplicar** (el calendario responde al instante aunque el dataset tenga miles de filas) y, mientras tanto, la grilla y el pie siguen mostrando lo mismo; al aplicar, la vista vuelve a la **página 1** porque el número de filas cambia. El panel trae además el botón **Limpiar** (quita el filtro) y el calendario los botones **Hoy** y **Limpiar**. En las columnas de fecha **solo existe la coincidencia total**: no hay selector `y`/`o` ni reglas adicionales, porque un rango ya se combina con el resto de filtros de la grilla. El rango también se respeta al **exportar a Excel** (§7.7) y el botón *Limpiar filtros* lo quita junto con el resto de filtros.
...
| `ModernDataGrid/helpers/ColumnFilters.ts` | Filtros de columna **compilados** que reproduce la exportación a Excel (campo, operador y predicado resueltos una sola vez) |
...
- **Rango de fechas: el valor tiene que llegar a `onFilter` (desde `1.0.0.40`; aplicado con el botón Aplicar desde `1.0.0.41`)**: el modelo de filtros lo controla el componente (`filters` + `onFilter`) y el `DataTable` filtra las filas con **sus props** (`getFilters()`), no con su copia interna (`d_filtersState`). Escribir el rango solo con `options.filterCallback` actualiza esa copia interna y **la grilla no filtra nada** (se veía el rango en el calendario y todas las filas en la tabla). La aplicación real es la del botón **Aplicar** (`showApplyButton` solo en las columnas de fecha): el clic del calendario mantiene el modelo del panel (`filterCallback`) y `onFilterApplyClick` escribe el rango en el estado del control (`setDateRangeFilterValue()`, que fuerza `matchMode = between` y no repinta si el rango es el mismo). El aviso diferido de PrimeReact (`filterDelay`, 300 ms) llega después con el mismo modelo y `onFilterChange()` lo ignora, así que **Aplicar es instantáneo**. En las columnas de fecha se desactivan el selector de operador y las reglas adicionales (`showFilterOperator` / `showAddButton`), y el panel muestra el resumen del rango elegido (`dateRangeSelected` + `dateRangeFormat` del idioma).
- **Los filtros de columna no revalidan el dataset (desde `1.0.0.41`)**: `componentDidUpdate()` ya **no** incluye `filtersChanged` en `structuralChange`, así que aplicar o limpiar un filtro no llama a `mapRecordsToState()` ni a `forceRefreshDataset()` y, por tanto, no dispara `notifyOutputChanged()` ni una recarga de la fuente: los filtros se aplican **en cliente** sobre las filas ya mapeadas. Antes, cada filtro aplicado hacía un viaje al host + un re-mapeo forzado + un `forceUpdate()`: eso es lo que volvía lento cada clic del calendario. El repintado lo sigue disparando `shouldComponentUpdate` (compara la firma del modelo de filtros), `onFilterChange()` ignora los avisos con el mismo modelo y, cuando el modelo cambia de verdad, la vista vuelve a la **página 1** (`currentPage = 1`, `pendingPage = null`) para no quedar en una página que ya no existe.
...
- **Mapeo incremental (desde `1.0.0.32`)**: cada fila formateada se guarda en `mappedRowCache` por `recordId` junto a la **referencia** del record del dataset. Si el record es el mismo objeto y la estructura no cambió (firma de las siete propiedades de formato + `Language` + columnas del dataset como `nombre:alias:tipo`), la fila se **reutiliza** sin volver a formatear (`reusedRows`). Se reformatea **todo** cuando cambia la estructura, cuando la app pide recargar (`invalidateMappedRows`, al pulsar *Actualizar*) o cuando el host revalida con `force` y la firma de filas no cambió. **No se usa `lodash.isEqual`**: la detección de cambios es por firma de filas (`loading`, cantidad, primer y último id) y por comparación de referencias.
...
- **Filtros de columna compilados (desde `1.0.0.38`)**: la exportación a Excel recorre las filas con los filtros **ya resueltos** (`helpers/ColumnFilters.ts`): campo del registro, operador y predicado de `FilterService` se calculan una sola vez por cambio del modelo (`getCompiledColumnFilters()`, memoizado por la firma de los filtros). Antes el modelo se reconstruía en **cada fila** (claves, restricciones y búsqueda del modo de coincidencia) y el contador del pie serializaba el modelo con `JSON.stringify` en cada render: las dos cosas desaparecieron. El comportamiento no cambia (mismo `FilterService`, `CONTAINS` por defecto cuando la restricción no trae `matchMode`, `and`/`or` respetados y las restricciones vacías ignoradas).
- **Cachés por identidad, no por firma (desde `1.0.0.38`)**: las etiquetas de columna (`getColumnLabels()`), la clave de los encabezados (`getColumnHeaderKey()`) y los encabezados resueltos (`getColumnHeader()`, uno por columna) se comparan por **identidad del arreglo de columnas** más el texto de `ColumnLabels`, así que pintar 44 encabezados no vuelve a recorrer ni a construir firmas de las columnas en cada render.
...
8. Las propiedades de formato (`CurrencyFormats`, `DateFormats`, `DateTimeFormats`, `TimeFormats`, `NumberFormats`, `DecimalFormats`, `BooleanLabels`) se reflejan en los valores (moneda, decimales, Sí/No, fechas y horas). Cada propiedad de fecha se aplica **solo** a las columnas de su tipo: si una columna aparece en la propiedad de otro tipo, la consola lo avisa una vez y el formato **no** se aplica.
...
En la consola también queda `window.context` con el contexto del control (útil para inspeccionar `context.parameters` mientras se depura). El refresco periódico del dataset (`setInterval` de 5 s) **solo** se activa mientras `needsRefresh` está en curso y se detiene en cuanto termina: entre interacciones no queda ningún temporizador corriendo.
...
| El rango de fechas filtra un día de menos | Elige el día inicial y el final (los dos extremos están incluidos) y pulsa **Aplicar**: mientras falte un extremo, la grilla no se filtra y el panel lo recuerda en la línea de ayuda |
| Elijo el rango de fechas y la grilla no cambia hasta que pulso Aplicar | Es lo previsto desde `1.0.0.41`: el calendario solo guarda lo elegido (responde al instante aunque haya miles de filas) y el filtro entra al pulsar **Aplicar** (§7.2) |
| El calendario de fechas tardaba segundos en responder en cada clic | Corregido en `1.0.0.41`: cada clic ya no filtra ni revalida el dataset (antes cada filtro aplicado hacía un viaje al host + re-mapeo completo + repintado). Importa esa versión y cierra/reabre la app |
| Aplico un filtro y la grilla se queda en blanco | Corregido en `1.0.0.41`: al cambiar un filtro la vista vuelve a la página 1 (antes podía quedarse en una página que ya no existía) |
...
Con `AllowFiltering = true`, pulsa el embudo de una columna de fecha o de fecha y hora: se abre un **calendario de rango** que va escribiendo el rango elegido (*Rango seleccionado: 22/09/2026 → 25/09/2026*). Elige el día inicial y el final (ambos incluidos) y pulsa **Aplicar**: la grilla, el pie y el Excel quedan filtrados a ese rango (§7.2). El rango se puede quitar con el **Limpiar** del panel o con *Limpiar filtros* de la barra.
